var base_url = "http://development.luminogurus.com/heatmaps";
display_numbers = false;
$(document).ready(function(){
    $('body').append('<p class="button displayClicks" style="display:none;"><a href="#demo">Display Click Map</a></p>');
...
function sizeContent() {
    ...
    $("#clickmap-overlay").css("height", newHeight);
}
function doc_keyUp(e) {
    if (e.ctrlKey && e.keyCode==16) {
        display_numbers = false;
        $('.displayClicks').trigger('click');
    }else if(e.ctrlKey && e.keyCode==18){
        display_numbers = true;
        $('.displayClicks').trigger('click');
    }
}

function displayHeatMaps(clicks){
    var max_value = 1;
    ...
    var config = {
        element: document.getElementById("clickmap-overlay"),
        radius: 7,
        opacity: 50
    };
...
(function($) { 
    ...
    $.fn.saveClicks = function() { 
        $(this).bind('mousedown.clickmap', function(evt) { 
            $.ajax({
                url: base_url+'/Devs/saveClicks',
                dataType:'jsonp',
                type:'GET',
                data:{  
                    "data[Click][x_position]":evt.pageX,  
                    "data[Click][y_position]":evt.pageY,  
                    "data[Click][location]":escape(document.location) 
                }
            });
        }); 
    }; 
            ...
            success:function(html){
                if(display_numbers){
                    ...
                    $.each(html,function(key,value){
                        div += "<div style='left:"+value["x"]+"px;top:"+value["y"]+"px;position:absolute;'>"+value["count"]+"</div>"; 
                    });
                    $('#clickmap-overlay').append(div);
                    
                }else{
                    displayHeatMaps(html)
